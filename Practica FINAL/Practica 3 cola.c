/*Crea un programa para gestionar un inventario de automóviles utilizando un
TDA. El TDA debe contener información sobre cada automóvil, incluyendo modelo,
stock (entero), y precio (decimal). Desarrollar las siguientes funciones para:

Agregar un nuevo modelo de automóvil al inventario.
Mostrar el inventario por consola y generar un archivo txt llamado
"inventario.txt". Eliminar un modelo de automóvil del inventario. Calcular el
valor total del inventario. Ordenar de manera por stock de menor a mayor los
modelos disponibles en el inventario usando quicksort Diseña un menú de opciones
...
void Menu();
void AgregarAutomovil(Cola **, Cola **);
void ImprimirInventario(Cola *);
void EliminarAutomovil(Cola **, Cola **);
double CalcularValorTotal(Cola *);
...
void Menu() {
  ...
  do {
    ...
    printf("3) Eliminar el primer automovil ingresado\n");
...
void AgregarAutomovil(Cola **frente, Cola **fin) {
...
void ImprimirInventario(Cola *frente) {
  Cola *aux = frente;
  ...
  while (aux != NULL) {
    printf("%-25s | %-25s | %-10d | %.2f\n", aux->automovil.marca,
           aux->automovil.modelo, aux->automovil.stock, aux->automovil.precio);
    aux = aux->sig;
  }
  ...
  if (archivo != NULL) {
    ...
    while (aux != NULL) {
      fprintf(archivo, "%-25s | %-25s | %-10d | %.2f\n", aux->automovil.marca,
              aux->automovil.modelo, aux->automovil.stock,
              aux->automovil.precio);
      aux = aux->sig;
    }
    fclose(archivo);
...
void EliminarAutomovil(Cola **frente, Cola **fin) {
  Cola *aux = *frente;
  Automovil automovil = aux->automovil;
  *frente = aux->sig;
  if (*frente == NULL) {
    *fin == NULL;
  }
  ...
  printf("%-25s | %-25s | %-10d | %.2f\n", automovil.marca, automovil.modelo,
         automovil.stock, automovil.precio);
  printf("\nse ha eliminado\n");
}

double CalcularValorTotal(Cola *frente) {
  Cola *aux = frente;
  double suma = 0;
  double porauto = 0;
